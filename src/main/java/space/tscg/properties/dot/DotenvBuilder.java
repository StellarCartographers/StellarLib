 ...
 */
package space.tscg.properties.dot;
...
import space.tscg.properties.dot.internal.DotenvParser;
import space.tscg.properties.dot.internal.DotenvReader;
import space.tscg.properties.sys.Systemenv;
...
public class DotenvBuilder
{
    ...
    public Dotenv load() throws DotenvException
    {
        ...
        if(env.isEmpty())
        {
            Systemenv sysEnv = Systemenv.load();
            sysEnv.variables().forEach((t, u) -> env.add(new DotenvEntry(t, u.asString())));
        }
