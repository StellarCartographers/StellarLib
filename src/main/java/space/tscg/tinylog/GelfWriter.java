...
public final class GelfWriter implements Writer
{
    ...
    private static final EnumSet<LogEntryValue> BASIC_LOG_ENTRY_VALUES = EnumSet.of(LogEntryValue.DATE, LogEntryValue.LEVEL, LogEntryValue.MESSAGE);
     ...
     */
    public GelfWriter(final String server, final int port, final GelfTransports transport, final String hostname, final Set<LogEntryValue> requiredLogEntryValues, final Map<String, Object> staticFields, final int queueSize, final int connectTimeout, final int reconnectDelay, final int sendBufferSize, final boolean tcpNoDelay)
    {
        this.server = server;
        this.port = port;
    ...
    private void init()
    {
        ...
        final GelfConfiguration gelfConfiguration = new GelfConfiguration(remoteAddress).transport(transport).queueSize(queueSize).connectTimeout(connectTimeout).reconnectDelay(reconnectDelay).sendBufferSize(sendBufferSize).tcpNoDelay(tcpNoDelay);
    ...
    {
        this(p.getOrDefault("server", "localhost"), Integer.parseInt(p.getOrDefault("port", "12201")), GelfTransports.valueOf(p.getOrDefault("transport", "UDP")), p.getOrDefault("hostname", null), buildLogEntryValuesFromString(p.getOrDefault("additionalLogEntryValues", "EXCEPTION")),
            Arrays.stream(p.getOrDefault("staticFields", "").split(",")).map(s -> s.split(FIELD_SEPARATOR)).collect(Collectors.toMap(s -> s[0], s -> s[1])), Integer.parseInt(p.getOrDefault("queueSize", "512")), Integer.parseInt(p.getOrDefault("connectTimeout", "1000")),
            Integer.parseInt(p.getOrDefault("recoveryTimeout", "500")), Integer.parseInt(p.getOrDefault("sendBufferSize", "-1")), Boolean.parseBoolean(p.getOrDefault("tcpNoDelay", "false")));
    }
    ...
    void write(final GelfTransport gelfClient, final LogEntry logEntry) throws Exception {
        ...
        if (null != throwable) {
            ...
            for (StackTraceElement stackTraceElement : throwable.getStackTrace()) {
                ...
                String s3 = stackTraceElement.getFileName();
                int i1 = stackTraceElement.getLineNumber();
                try(Formatter formatter = new Formatter(stackTraceBuilder)) {
                    formatter.format("%s.%s(%s:%d)%n", s1, s2, s3, i1);
                }
            }
    ...
    private GelfMessageLevel toGelfMessageLevel(final Level level) {
        return switch (level)
        {
            case TRACE, DEBUG -> GelfMessageLevel.DEBUG;
